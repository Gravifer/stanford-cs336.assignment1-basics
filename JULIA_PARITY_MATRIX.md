...
| Grouped/multi-query attention | no direct adapter; implemented inside adapter-reachable attention family | 3 experiment | structurally validated execution lane | grouped contraction matches expanded-KV MHA without materializing repeated K/V; a correctness-gated MHA/GQA/MQA benchmark harness is ready, while retained timing trials and grouped CUDA remain pending |
...
| BPE training | `run_train_bpe`; tiny Python probes plus repository fixtures | 1 | tiny-corpus parity-validated | vocabulary bytes, merge order/tie-break, lazy heap with authoritative incremental counts, special boundaries, merge exhaustion, serial/threaded determinism; full-corpus run user-deferred |
